...
```json
{
  ...
  "architecture": {              // Hardware + runtime environment
    "machine": "string",        // platform.machine() (e.g., "x86_64")
    "processor": "string",      // platform.processor() or CPU model string
    "cpu_count": "number",      // os.cpu_count()
    "os": "string",             // OS + kernel (e.g., "Linux 6.8.0")
...
```json
{
  ...
  "architecture": {
    "machine": "x86_64",
    "processor": "Intel(R) Xeon(R) Gold 6330 CPU @ 2.00GHz",
    "cpu_count": 64,
    "os": "Linux 6.8.0",
...
The `architecture` object captures essential hardware and runtime details enabling cross-run performance normalization.

Field descriptions:

- `machine`: Output of `platform.machine()`.
- `processor`: CPU model string (`platform.processor()` or fallback logic).
- `cpu_count`: Logical CPU count from `os.cpu_count()`.
- `os`: Concatenation of `platform.system()` and `platform.release()` (or `platform.platform()` simplified).
- `python_version`: Interpreter version from `platform.python_version()`.

Additional optional fields MAY be introduced (e.g., memory_gb, numa_nodes). Consumers SHOULD ignore unknown fields for forward compatibility.
